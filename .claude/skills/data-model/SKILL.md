...
tournaments (1) ──→ (N) schedule_candidates (1) ──→ (N) schedule_responses ←── (N) players
...
Standing::activePlayersWithDetails($tournamentId);              // 勝ち抜き中の選手をDM送信用詳細付きで取得
...
TableInfo::searchAll($filters, $limit, $offset);                // 全卓フラット検索（tables.php 用、tournament_id/event_types/status/keyword）
TableInfo::searchAllCount($filters);                            // searchAll の件数（ページネーション用）
TableInfo::create($tournamentId, $roundNumber, $tableName, $playerIds); // 卓作成
TableInfo::createBatch($tournamentId, $roundNumber, $tables);   // 複数卓一括作成（$tables[]に played_date/day_of_week/played_time を含められる、任意）
TableInfo::nextRoundNumber($tournamentId);                      // 次に作るラウンド番号（既存最大+1、卓未作成なら1）
...
### ScheduleCandidate
ラウンドごとに主催者が登録する候補日程（例: 6/22昼, 6/22夜...）。卓作成前の日程調整アンケート用。

```php
ScheduleCandidate::byRound($tournamentId, $roundNumber);            // ラウンドの候補日程一覧（sort_order順）
ScheduleCandidate::createBatch($tournamentId, $roundNumber, $candidates); // 一括置き換え（全削除→再作成、既存回答もCASCADE削除）
ScheduleCandidate::responseCountsByRound($tournamentId, $roundNumber); // candidate_id => 回答人数
```

### ScheduleResponse
選手が ScheduleCandidate から選んだ「参加可能」回答（不参加は単に未選択、行を作らない）。

```php
ScheduleResponse::byCandidateIds($candidateIds);                    // candidate_id => [player_id,...]（自動生成アルゴリズムの入力）
ScheduleResponse::byPlayer($tournamentId, $roundNumber, $playerId); // 選手の回答済みcandidate_id一覧
ScheduleResponse::replaceForPlayer($tournamentId, $roundNumber, $playerId, $candidateIds); // 選手の回答を一括置き換え
ScheduleResponse::respondedPlayerIds($tournamentId, $roundNumber);  // 1つ以上回答済みの選手ID一覧
```
